...
const create=async(req,res,next)=>{
    try {
        ...
        if(userExists){
            return res.status(400).json({success:false,message:"User Alread Exists"})
        }else{
...
const update=async(req,res,next)=>{
    try {
        ...
        if(user){
            ...
            return res.status(200).json({success:true,message:"Successfully Updated the user",data:updatedUser})
        }else{
